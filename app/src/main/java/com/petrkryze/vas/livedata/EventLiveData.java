...
public  class EventLiveData<T> extends LiveData<T> {
    ...
    public EventLiveData() {
        this.internalObserver =  (t -> {
            for (Map.Entry<Observer<? super T>, EventObserverWrapper> observerEventObserverWrapperEntry : EventLiveData.this.observers.entrySet()) {
                EventObserverWrapper wrapper = observerEventObserverWrapperEntry.getValue();
    ...
    @MainThread
    public void observe(@NonNull LifecycleOwner owner, @NonNull Observer observer,
                        @NonNull Lifecycle.State minimumStateForSendingEvent,
                        Lifecycle.Event removeObserverEvent) {
    ...
    public void observeForever(@NonNull Observer observer) {
        ...
        EventObserverWrapper existing = wrapper;
        if(!observers.containsKey(observer))existing = observers.put(observer, wrapper);
        if (existing instanceof EventLiveData.EventLifecycleBoundEventObserver) {
            throw new IllegalArgumentException("Cannot add the same observer"
                    + " with different lifecycles");
        }
        if (existing != null) {
